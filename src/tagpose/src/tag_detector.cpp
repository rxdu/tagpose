 ...
 * Description:
 *
 * Copyright (c) 2017-2020 Ruixiang Du (rdu)
 */

#include "tagpose/tag_detector.hpp"
#include "apriltag_utils/PoseUtil.h"

namespace al {
TagDetector::TagDetector()
    : tag_size_(0.064), cam_fx_(595), cam_fy_(595), cam_px_(330), cam_py_(340) {
...
AprilTags::TagDetection TagDetector::ConvertDetectionStruct(
...
void TagDetector::SetCameraParams(double fx, double fy, double px,
                                     double py) {
  cam_fx_ = fx;
  cam_fy_ = fy;
  cam_px_ = px;
...
void TagDetector::LookForApriltags(cv::InputArray _src, bool print_output) {
  ...
  // std::cout << "image size: " << frame.rows << " , " << frame.cols << " ; tag
  // detected: " << zarray_size(detections) << std::endl;
  ...
  for (int i = 0; i < zarray_size(detections); i++) {
    ...
    tag_det.getRelativeTranslationRotation(tag_size_, cam_fx_, cam_fy_, cam_px_,
                                           cam_py_, translation, rotation);
